...
class XerusHandler():
    ...
    def ticket_header(self, ticket: str) -> ASObject:
        """
        Generate a ticket header for the given ticket
        """

        marking_id = self.get_marking_id()
    ...
    def calculate_checksum(self, arguments: Union[int, str, bool, bytes, List[Union[int, str, bool, bytes]],
    ...
    def invoke_method(self, server: str, method: str, params: list, session_id: str) -> tuple[int, any]:
        """
        Invoke a method on the MSP API
        """
        proxy = None

        if self.use_proxies and self.proxies != None:
            proxy = self.proxies
        ...
        event.headers = remoting.HeaderCollection({
            ...
            ("id", False, self.calculate_checksum(params)
        )})
...
def _example():
    ...
    code, resp = handler.invoke_method(
        SERVER,
